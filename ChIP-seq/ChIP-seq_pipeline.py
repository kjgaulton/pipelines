...
def process_reads(args):
	...
	bwa_mem_cmd = ['bwa', 'mem', '-t', str(args.processes), args.reference, args.reads]
...
def main(args):
	...
	if not args.skip_align:
		try:
			...
			print('Check options -r: ' + repr(e), file=sys.stderr)
...
def process_args():
	...
	parser.add_argument('-r', '--reads', required=True, type=str, help='Path to reads (.fastq.gz)')
	...
	parser.add_argument('-ref', '--reference', required=True, type=str, help='Path to reference genome')
	parser.add_argument('-markdup', '--markdup', required=True, type=str, help='Path to MarkDuplicates.jar')
	parser.add_argument('-p', '--processes', required=False, type=int, default=12, help='Number of processes to use')
	parser.add_argument('-m', '--memory', required=False, type=int, default=12, help='Maximum memory per thread for samtools sort')
